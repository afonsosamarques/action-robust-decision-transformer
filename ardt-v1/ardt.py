...
from transformers import DecisionTransformerModel, DecisionTransformerGPT2Model, DecisionTransformerOutput
...
class DecisionTransformerGymDataCollator:
    ...
    def __init__(self, dataset, context_size, returns_scale):
        ...
        self.pr_act_lb = min(0, (int(np.min([traj["pr_actions"] for traj in dataset])) - 1) * 5.0)
        self.adv_act_lb = min(0, (int(np.min([traj["adv_actions"] for traj in dataset])) - 1) * 5.0)
    ...
    def __call__(self, features):
        ...
        return {
            "states": s,
            "protagonist_actions": a_pr,
            "adversary_actions": a_adv,
            "rewards": r,
            "returns_to_go": rtg,
            "timesteps": tsteps,
            "attention_mask": mask,
...
class AdversarialDT(DecisionTransformerModel):
    ...
    def forward(
        ...
        if is_train:
            ...
            # return predictions
            if not return_dict:
                return (alpha_preds, epsilon_preds, adv_action_preds)

            return DecisionTransformerOutput(
                alpha_preds=alpha_preds,
                epsilon_preds=epsilon_preds,
