...
from .config_utils import load_model, load_adv_model
...
def evaluate(
    ...
    for alpha in [0.1, 0.3, 0.5]:
        ...
        try:
            adv_model, is_adv_adv_model = load_adv_model(adv_model_type, adv_model_name, model_path=(adv_model_path if adv_model_path is not None else hf_project + f"/{adv_model_name}"))
        except HTTPError as e:
        ...
        # evaluation loop
        print("\n================================================")
        print(f"Evaluating protagonist model {pr_model_name} on environment {env_type} against adversarial model {adv_model_name} on alpha {alpha}.")
        ...
        with torch.no_grad():
            try:
                return_target = pr_model.model.config.max_ep_return
            except Exception:
                return_target = eval_target
        
            pr_model.new_batch_eval(start_states=start_states, eval_target=return_target)
            ...
            for t in range(env_steps):
                ...
                cumul_actions = (pr_actions + alpha * adv_actions)
        ...
        dir_path = f'{find_root_dir()}/eval-outputs{run_suffix}/{env_type}/{pr_model_name}'
        if not os.path.exists(dir_path):
            os.makedirs(dir_path)
        subdir_path = f'{dir_path}/{adv_model_name}'
        if not os.path.exists(subdir_path):
            os.makedirs(subdir_path)
        with open(f"{subdir_path}/{alpha}.json", 'w') as f:
            json.dump(eval_dict, f)

        # save data_dict as hf dataset
        if record_data:
            data_ds = Dataset.from_dict(data_dict)
            data_ds.save_to_disk(f'{find_root_dir()}/datasets/{pr_model_name}_{adv_model_name}_{alpha}_eval_{env_type}')
