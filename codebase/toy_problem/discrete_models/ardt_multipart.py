...
from .returns_oracle import ReturnsOracle
...
class ProtagonistDT(DecisionTransformerModel):
    def __init__(self, config, logger=None):
        ...
        self.logger = logger
        self.step = 0
        self.oracle = ReturnsOracle(config.env_version)
    ...
    def forward(
        ...
        # we condition on wc returns; call oracle to obtain them
        if is_train:
            wc_rtg = []
            cumul_actions = np.concatenate((pr_actions.reshape(-1, self.config.pr_act_dim).detach().cpu().numpy(), adv_actions.reshape(-1, self.config.adv_act_dim).detach().cpu().numpy()), axis=1)
            for i in range(len(cumul_actions)):
                action = cumul_actions[i]
                wc_rtg.append(self.oracle(action))
            wc_rtg = torch.tensor(wc_rtg, dtype=torch.float32, device=states.device).reshape(-1, 1, 1)
        else:
            wc_rtg = returns_to_go

        # embed each modality with a different head
        state_embeddings = self.embed_state(states)
        pr_action_embeddings = self.embed_pr_action(pr_actions)
        returns_embeddings = self.embed_return(wc_rtg)
...
class MultipartARDT(DecisionTransformerModel):
    ...
    def get_actions(self, states, pr_actions, adv_actions, rewards, returns_to_go, timesteps, device, batch_size=1):
        ...
        # a ones mask is not appropriate here because it coincides with one of the actions; fix it
        pr_actions = torch.ones_like(pr_actions, device=states.device) * torch.tensor(0.5, device=states.device)
        adv_actions = torch.ones_like(adv_actions, device=states.device) * torch.tensor(0.5, device=states.device)
