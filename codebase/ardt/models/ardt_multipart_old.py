...
# NOTE keep to be used as adversary given it has already been trained, despite with mixed results.
...
class AdversarialDT(DecisionTransformerModel):
    ...
    def forward(
        ...
        rtg_sample = rtg_sample.rsample((batch_size,))
        ...
        if is_train:
            ...
            rtg_entropy = -rtg_dist.log_prob(rtg_sample).mean(axis=0).sum()
            ...
            return {"loss": rtg_loss + self.config.lambda2 * adv_action_loss,
                    ...
                    "mu": mu_preds,
                    "sigma": sigma_preds,
                    "rtg_preds": rtg_dist.icdf(torch.tensor([0.01], device=stacked_inputs.device)),
                    "adv_action_preds": adv_action_preds}
        else:
            # return predictions
            if not return_dict:
                return (rtg_dist.icdf(torch.tensor([0.01], device=stacked_inputs.device)), adv_action_preds)
...
class StochasticDT(DecisionTransformerModel):
    ...
    def forward(
        ...
        pr_action_sample = pr_action_dist.rsample((batch_size,))

        if is_train:
            # return loss
            pr_action_log_prob = pr_action_dist.log_prob(pr_actions).sum(axis=2)[attention_mask > 0].mean()
            pr_action_entropy = -pr_action_dist.log_prob(pr_action_sample).mean(axis=0).sum(axis=2).mean()
            ...
            # return predictions
            if not return_dict:
                return (pr_action_dist.icdf(torch.tensor([0.5], device=stacked_inputs.device)))
...
class MultipartARDTOld(DecisionTransformerModel):
